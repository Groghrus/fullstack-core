---
id: slo
title: SLO (Целевые уровни обслуживания)
...
SLO задаются как набор явных целей и сравниваются с фактическими SLI в мониторинге:

```ts
const slos = {
  availability: { target: 99.9, windowDays: 30 },
  latency: { targetPercentile: 99, thresholdMs: 200 },
}
```

Цели проверяются автоматически: если SLI ниже цели, остаток бюджета тает и релизы приостанавливаются — не по ощущениям, а по числам.

## Паттерны использования

- **SLI → SLO 1:1** — на каждую цель одна детерминированная метрика.
- **Реалистичные цели** — 99.9% на уровне B2C и требование «пять девятки» за другие деньги — разные решения.
- **Окно 28–30 дней** — стабильно и сравнимo с бюджетом ошибок.
- **SLO проверяется автоматически** — сравнение SLI с целью в алертинге, а не вручную раз в квартал.

## Антипаттерны и ловушки

- **SLO выше реальности** — недостижимая цель никого не мотивирует и обесценивает budget.
- **Слишком много SLO** — каждая цель требует поддержки, теряется фокус.
- **SLO = 100%** — недостижимо, бюджет нулевой, релизы встанут навсегда.
- **SLO без автоматического трекинга** — цель «на бумаге» не влияет на процесс.

## Когда использовать / когда НЕ использовать

- **Использовать:** сервисы с постоянным трафиком и регулярными изменениями, где нужен контракт между командой и бизнесом.
- **НЕ использовать:** для экспериментов и внутренних утилит без требований к доступности — начало с SLO закрывает актуальный мониторинг и алерты.

## Связанные темы

- **sli** — метрики, на которых строятся цели.
- **error-budgets** — как SLO превращается в управление рисками релизов.
- **monitoring** — источник данных для проверки целей.
