---
id: error-budgets
title: Бюджеты ошибок (Error Budgets)
...
Превращаем остаток бюджета в CI-gate: деплой блокируется, если бюджет исчерпан.

```ts
async function canDeploy(): Promise<boolean> {
  const sli = await prometheus.query(
    `100 - (sum(rate(errors_total[30d])) / sum(rate(total[30d])) * 100)`,
  )
  return sli >= SLO_TARGET // 99.9% — бюджет в плюсе
}
```

Оценка считается по тому же окну, что и SLO, и интегрируется в пайплайн на этапе перед деплоем — политика становится выполняемой автоматически.

## Паттерны использования

- **Считать от SLO** — остаток бюджета это `SLO − текущий SLI`, а не произвольное «всё ок».
- **CI-gate на каждый деплой** — блокировка в пайплайне, а не на совести релиза.
- **Дашборд остатка за окно** — budget виден команде, а не хранится в «голове».
- **Заморозка при исчерпании** — пока бюджет не восстановился, только багфиксы и откаты.

## Антипаттерны и ловушки

- **SLO без бюджета** — цифры не влияют на решения, остаются «нормативом на бумаге».
- **Трактовка бюджета на глаз** — 43 минуты в месяц легко «простить» без формальной политики.
- **Ручное перекрытие бюджета для «важного» релиза** — доверие к процессу сгорает.
- **Бюджет, посчитанный вне реальных данных** — пиковые инциденты «прячутся» за средним.

## Когда использовать / когда НЕ использовать

- **Использовать:** когда есть SLI/SLO и регулярные деплои; чтобы балансировать скорость релизов и стабильность.
- **НЕ использовать:** без SLO бюджет бессмыслен; для систем без требований к надёжности.

## Связанные темы

- **slo** — целевые значения, из которых выводится бюджет.
- **sli** — фактическая метрика, по которой считается остаток.
- **monitoring** — источник данных для оценки бюджета.
