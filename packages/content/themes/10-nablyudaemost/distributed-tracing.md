...
Подключаем авто-инструментацию OpenTelemetry: трейсы уходят в Collector, `traceparent` распространяется между сервисами автоматически:
...
Достаточно, чтобы `traceId` пролетел через HTTP-цепочку; hand-written спаны добавляются через `tracer.startSpan()` там, где нужен ручной контекст (очередь, batch).

## Паттерны использования

- **W3C `traceparent` на всех клиентах и серверах** — стандартная кросс-сервисная передача контекста.
- **Именование спанов по операции, лейблы по зависимостям** — быстрый поиск источника задержки.
- **Сэмплинг по приоритету** — быстрые запросы прореживаются, медленные и ошибки пишутся целиком.
- **Единый OTel Collector** — трейсы, метрики и логи стекаются в одну точку и коррелируются по traceId.

## Антипаттерны и ловушки

- **Трассировка без охвата БД и очередей** — bottleneck в SQL остаётся невидимым.
- **Разрыв контекста на асинхронных задачах** — batch-джобы и очереди теряют `traceparent`.
- **100% сэмплинг** — хранение всего дорого; сбор редко отключается и теряет ценность.
- **Секреты в атрибутах спанов** — запросы/строки подключения попадают в хранилище как есть.

## Когда использовать / когда НЕ использовать

- **Использовать:** микросервисы из двух и более сервисов, поиск латентности в цепочке, разбор инцидентов, SLO по времени ответа.
- **НЕ использовать:** монолит с одним процессом без внешних вызовов, короткие функции без сетевых зависимостей, где проще обычные логи.

## Связанные темы

- **monitoring** — метрики для алертов, трейсы для причин.
- **logging** — логи коррелируются с трейсами по traceId.
- **metrics** — числовые агрегаты, дополняющие путь запроса.
