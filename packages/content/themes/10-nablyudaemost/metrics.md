...
Выставляем метрики приложения через prom-client в endpoint `/metrics`:
...
Prometheus забирает показания по расписанию (pull-модель) — приложение не отправляет их само.

## Паттерны использования

- **Правильные типы** — counter для событий, gauge для состояния, histogram для распределений.
- **Умеренная кардинальность лейблов** — method/status/route, но не `user_id` или `request_id`.
- **Pull-модель `/metrics`** — единый контракт для Prometheus и всего остального.
- **Нейминг по конвенции** — `_total`, `_seconds`, `_bytes` понятны инструментам и коллегам.

## Антипаттерны и ловушки

- **Взрыв кардинальности** — уникальный лейбл на каждый запрос кладёт TSDB и грабители Prometheus.
- **Метрика как «чёрный ящик» без дашборда и алерта** — данные собираются и никем не смотрятся.
- **Синонимы для одного показателя** — два названия одной метрики в разных сервисах путают запросы.
- **Переполнение/утечка серий** — забытые метрики из отключённых фич растут бесконечно.

## Когда использовать / когда НЕ использовать

- **Использовать:** алерты, дашборды, autoscaling, расчёт SLI/SLO, сравнение релизов.
- **НЕ использовать:** для детального отладочного контекста (логи/трейсы лучше), для миллиарда разномастных лейблов.

## Связанные темы

- **monitoring** — метрики это сырьё для мониторинга и алертов.
- **logging** — логи дают контекст, который не выжимается из метрик.
- **distributed-tracing** — трейсы показывают путь запроса, метрики — тренды.
