...
public class VersionCheck {
    public static void main(String[] args) {
        ...
        System.out.println(current.nextMinor());            // 1.3.0
        System.out.println(current.isGreaterThan("1.0.0")); // true
        System.out.println(current.satisfies(">=1.0.0"));   // true (диапазон)
    }
}
```
...
### JavaScript (semantic-release: версия из типа коммита)
...
```go
// go.mod — требование точных версий модулей
...
// go get github.com/user/lib@latest   (последняя опубликованная)
```
...
```markdown
<!-- бейдж / счётчик зависимостей в шапке репозитория -->
...
- **Лепить `0.1.x` вечно** — контракт уже ведёт себя как нестабильный, но вы боитесь `1.0.0`; потребители не могут планировать.
...
- Плановые релизы в монорепо с автоматизацией (semantic-release).
...
Пояснение: caret разрешает любые minor/patch, не нарушающие контракт: мажорная не меняется.

### Q5
**Почему `0.x` версии «нестабильны» по SemVer?**
- [ ] Их нельзя парсить
- [x] До `1.0.0` публичный API считается неконтрактным, под **любое** изменение
