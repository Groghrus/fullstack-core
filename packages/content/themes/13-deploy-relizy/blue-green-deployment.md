...
Переключение трафика — смена селектора Service'а с Blue на Green:
...
spec:
  selector:
    app: api
    release: green   # было: blue
  ports:
    - port: 80
      targetPort: 8080
```

Green готов и проверен, Service переключается мгновенно; при проблемах одна смена селектора возвращает Blue — без повторного деплоя.

## Паттерны использования

- **Две полноценные среды** — Blue и Green одинаковы по конфигурации и зависимостям.
- **Проверка Green до переключения** — smoke-тесты и health checks на новой версии.
- **Смена трафика одним шагом** — селектор/DNS/LB-роутинг, а не передеплой.
- **Rollback = переключение обратно** — старая версия остаётся готовой.

## Антипаттерны и ловушки

- **Green недостаточно протестирован** — момент переключения становится лотереей.
- **Раздельные миграции БД** — схема Blue несовместима со схемой Green: переключение ломает данные.
- **Не поддерживать Blue после старта** — rollback требует работающей старой среды.
- **Усугублённый расход ресурсов** — две среды живут одновременно: цена оправдана, но контроль обязателен.

## Когда использовать / когда НЕ использовать

- **Использовать:** критические сервисы, где downtime недопустим и мгновенный откат ценен; когда есть ресурсы на вторую среду.
- **НЕ использовать:** для частых мелких обновлений — две среды на каждую итерацию дорого; когда миграции схем не обратимы между версиями.

## Связанные темы

- **canary-releases** — постепенный выкат вместо полного переключения.
- **rolling-deployments** — поэтапное обновление без второй среды.
- **rollbacks** — механизмы возврата к стабильной версии.
