...
**Dependency Hell (Ад зависимостей)** — состояние проекта, когда управление пакетами/библиотеками превращается в хаос: конфликтующие версии транзитивных зависимостей, несовместимые контракты, огромный и неконтролируемый граф, «чужой» код внутри по непонятной причине. Систематическая причина — **семантическая ложь** версий (SemVer нарушается), конфликтующие диапазоны и распределённость графа (A→B→C, B→D, C→D разных версий).
...
Пакетный менеджер выстраивает **граф зависимостей**: прямое объявление (`package.json`, `go.mod`, `pom.xml`) → транзитивные зависимости → их транзитивные → ... Пакетный менеджер:

- **SemVer-диапазоны** разрешаются в конкретные версии согласно lockfile (`package-lock.json`, `yarn.lock`, `go.sum`).
...
- Автоматические major-обновления без детального код-ревью — только patch/minor.
...
- [ ] Оставить навечно
- [x] Проанализировать changelog/breaking changes, прогнать тесты и канареечный деплой
- [ ] Просто поменять номер

Пояснение: major — потенциально ломающий; нужен changelog, регресс и контролируемый выкат.
