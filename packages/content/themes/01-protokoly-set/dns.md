...
- **Виртуальные хосты и переадресация** — один сервер может обслуживать много доменов (SNI/виртуальные хосты), домен может быть `CNAME`-алиасом (напр. `www` → apи CDN).
- **Балансировка и отказоустойчивость** — DNS round robin, geoDNS, health-check через DNS-записи (CDN/эни-сеть).
