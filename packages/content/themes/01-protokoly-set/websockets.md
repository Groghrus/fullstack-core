...
sequenceDiagram
    ...
    C->>S: сообщение (JSON)
    S-->>C: ответ (JSON)
    S-->>C: push от сервера (JSON) — сервер шлёт сам
    C->>S: ping
    S-->>C: pong
    C->>S: close (1000)
    S-->>C: close (1000)
...
2. **Кадры (frames)**: текстовые (opcode 1), бинарные (opcode 2), ping (9), pong (10), close (8). Опциональная фрагментация (длительные сообщения).
...
5. **URL**: `ws://` / `wss://` (TLS). Всё то же, что при HTTPS — шифрование (TLS).
...
- **Дублирование reconnect-логики** — reconnect у клиента и у gateway живёт в разных местах и «ломает» соединение; нужен единый модуль переподключения.
...
- Простая страница «обновить список данных» — обычного REST/polling достаточно, WebSocket избыточен.
- Обычные request/response — REST/HTTP проще, кешируется, совместим с прокси/CDN.
- Длинные фоновые рассылки, где клиент ничего не шлёт, — чаще проще **Server-Sent Events** (односторонние, авто-переподключение, проще).
- Тихие каналы с редкими сообщениями — соединение держится ради редких push; без двустороннего обмена проще SSE/polling.
...
- **HTTP/2 и HTTP/3** — WebSocket долго жил на HTTP/1.1-upgrade; HTTP/2 меняет мультиплексирование (extended CONNECT); QUIC/HTTP/3 выигрывает там, где важна низкая задержка.
...
Пояснение: сеть может оборваться без close-кадра; ping/pong подтверждает «я жив», и сервер закрывает молчащие соединения (timeout).
