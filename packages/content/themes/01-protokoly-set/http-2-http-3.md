...
- **HTTP/1.1 ограничен**: по одному запросу за соединение (head-of-line) → или много TCP-соединений (по 6-8 на страницу), или очередь запросов. Латентность растёт с числом ресурсов.
- **HTTP/2** — мультиплексирование в одном соединении: десятки запросов параллельно без новых рукопожатий; меньше рукопожатий, сжатые заголовки.
...
sequenceDiagram
    ...
    Note over C,S: HTTP/1.1 — 3 запроса: 3 соединения (или очередь в одном — HOL)
...
- **Мультиплексирование** — потоки независимы, запросы не ждут друг друга; потоки управляются приоритетами/весами.
...
- **Только TLS 1.3** — шифрование встроено в QUIC (в HTTP/2 TLS подключается отдельно от транспорта).
...
// Go автоматически включает HTTP/2 поверх TLS (ALPN h2);
// для HTTP/1.1 достаточно простого listen-and-serve.
func main() {
...
// ALPN согласовывается при TLS-рукопожатии; http2.connect создаёт h2-клиента,
// если сервер не поддержит h2 — придёт ошибка ERR_ALPN_NPN_PROTOCOL_NOT_FOUND.
function observeProtocol(url: string) {
  const client = http2.connect(url, { settings: { maxConcurrentStreams: 64 } })
  client.on('error', (err) => {
    console.log('HTTP/2 недоступен:', (err as NodeJS.ErrnoException).code ?? err.message)
  })
  return client // дальше — client.request(...), это уже HTTP/2
}
```
...
- **Ограничить concurrency потоков** — на сервере лимит `maxConcurrentStreams`; не полагаться на Server Push (часто отключён клиентами).
...
- HTTP/3: мобильные и сети с потерями, стриминг/реалтайм, долгие соединения и приложения, чувствительные к задержке.
...
- **Load Balancing** — L7-балансировщики должны понимать h2/h3 (иначе правила маршрутизации разрывают мультиплекс).
- **Security** — TLS 1.3 обязателен в HTTP/3; правильная настройка важна для h2/h3.
