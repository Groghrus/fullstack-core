...
**Long Polling (Длинный опрос)** — техника получения обновлений поверх обычного HTTP: клиент отправляет запрос, а сервер **не отвечает сразу**, а удерживает соединение до тех пор, пока не появится новое событие (или не истечёт таймаут удержания). Как только сервер отвечает — клиент **немедленно отправляет следующий запрос**. Так формируется «почти push» без WebSocket, без `Upgrade`, на любым HTTP/1.1.
...
4. **Цикл непрерывен**: соединение как бы «дышит» — редко создаётся, быстро переподключается; клиент никогда не бывает «без запроса».
...
export function publish(ev: unknown) {
  journal.push(ev)
  while (waiting.length) waiting.shift()!(ev) // будим одного ожидающего
}
```
...
- **Использовать для очень частых событий** — при высокой частоте обновлений long-polling — просто дорогое короткое опробование: лучше стрим (SSE/WS).
...
- Когда доступен **Server-Sent Events** — он проще и умеет автоперподключение (см. SSE).
...
- [ ] Меньше задержка до события и меньше пустых запросов
- [x] Ответ приходит сразу после события (а не к ближайшему тику); меньше трафик пустых ответов
- [ ] Быстрее сеть
- [ ] Не нужен HTTP
...
Пояснение: бесконечно держать GET нельзя — прокси и балансировщики рвут; короче серверный HOLD + переповтор = надёжный цикл.
