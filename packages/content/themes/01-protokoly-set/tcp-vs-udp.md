...
**UDP (User Datagram Protocol)** — лёгкий **дейтаграммный** транспорт «без состояния»: отправили дейтаграммы и забыли — без гарантий доставки, порядка, целостности потока. Быстрый, малозадержен, не держит соединение. Поверх него — DNS, VoIP/стриминг, онлайн-игры, мониторинг, QUIC (HTTP/3).
...
> Реальный выбор транспорта: надёжный TCP для бизнес-данных, лёгкий UDP для метрик/стриминга, QUIC (UDP) — когда нужен низкий latency + надёжность, но latency не хочется платить за TCP.
...
- **UDP — когда latency/поток важнее прощения** — видео/голос (дроп кадра лучше, чем задержка), онлайн-игры, мониторинг, DNS, мультикаст.
- **QUIC/HTTP/3 (UDP)** — берём надёжность TCP-уровня, но без head-of-line blocking: для приложений с high-latency-чувствительностью (см. HTTP/2 vs HTTP/3).
- **Фрейминг поверх TCP** — определяем собственные границы сообщений: длина-префикс, разделители, JSON-lines (ser/ser и ser/cor в Go).
...
- [ ] UDP шифрует данные
- [ ] TCP работает только внутри интернет сети
