---
id: throughput
title: Throughput (Пропускная способность)
...
Прогоняем нагрузочный сценарий в CI перед релизом (k6) и сравниваем RPS с baseline:
...
Падение RPS при неизменном числе VU — сигнал saturation: система перестала масштабироваться, дальше Latency начнёт расти.

## Паттерны использования

- **Замер при фиксированном числе VU** — честное сравнение между релизами.
- **Закон Литтла как каркас планирования** — из числа запросов в системе и времени ожидания выводится нужный Throughput.
- **Насыщение как граница capacity planning** — за точкой насыщения рост RPS обрывается.
- **Autoscaling по saturation, а не только по CPU** — очередь и соединения сигналят раньше процессора.

## Антипаттерны и ловушки

- **Максимизировать RPS без порога Latency** — система «держит» нагрузку, но отвечает через секунды.
- **Нагрузочный прогон на ноутбуке** — результаты не воспроизводятся на выделенном стенде.
- **Смешение тестового и бизнес-трафика в проде** — показатели искажаются событиями реальных пользователей.
- **Считать Throughput вместе с ошибками** — 5000 RPS, из которых 3000 — 503, не является пропускной способностью.

## Когда использовать / когда НЕ использовать

- **Использовать:** capacity planning, выбор размера инстансов, проверка эффекта оптимизации, настройка autoscaling-порогов.
- **НЕ использовать:** как единственную метрику без Latency и error rate; для интерактивных UX-требований, где важнее перцентили задержки.

## Связанные темы

- **latency** — скорость одного запроса, обратная сторона Throughput.
- **p99-latency** — как задержка ведёт себя в хвосте при росте нагрузки.
- **tail-latency** — хвост распределения как ограничение реальной пропускной способности.
