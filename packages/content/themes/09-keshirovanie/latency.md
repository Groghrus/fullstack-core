...
Измеряем задержку на границе сервиса — middleware пишет время ответа в гистограмму для дальнейшего расчёта перцентилей:

```ts
app.use((req, res, next) => {
  const end = observeLatency.startTimer({ route: req.path })
  res.on('finish', () => end({ status: res.statusCode }))
  next()
})
```

Метрика попадает в гистограмму (например, Prometheus histogram), из которой строятся P50/P95/P99 — уже в темах p99-latency и tail-latency.

## Паттерны использования

- **Измерять на границе сервиса** — отдельно фиксируются сетевая и серверная составляющие.
- **Per-route гистограммы** — общий average маскирует медленные endpoint'ы.
- **Timeout как контракт** — клиентский таймаут выставляется меньше бюджета времени ответа.
- **Считать хвост, а не среднее** — среднее врёт на хвостатых распределениях.

## Антипаттерны и ловушки

- **Average как единственный показатель** — 99% быстрых ответов скрывают катастрофу в 1%.
- **Игнорирование queueing delay** — «сервер быстрый», но очередь перед ним растёт бесконечно.
- **Слишком длинные таймауты** — в цепочке вызовов они мульплицируют нагрузку при деградации.
- **Замер только серверного времени** — пользователь ждёт ещё загрузку и рендер на клиенте.

## Когда использовать / когда НЕ использовать

- **Использовать:** UX-критичные запросы, выполнение SLO, сравнение релизов, поиск bottleneck'ов в цепочке вызовов.
- **НЕ использовать:** фоновые batch-задачи, где важен throughput, а не время одного вызова; как единственную метрику здоровья системы (нужны ещё error rate и saturation).

## Связанные темы

- **throughput** — вторая сторона нагрузки: сколько запросов система выдерживает.
- **p99-latency** — как измерять задержку в перцентилях вместо среднего.
- **tail-latency** — хвост распределения и борьба с ним.
