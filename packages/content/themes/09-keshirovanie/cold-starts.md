---
id: cold-starts
title: Cold Starts (Холодный старт)
...
Переносим тяжёлую инициализацию (пул соединений, конфиг) на этап загрузки модуля — она выполняется один раз при холодном старте, а не на каждый вызов:

```ts
// Инициализация на уровне модуля: платформа прогреет её при холодном старте
const db = new Pool({ connectionString: process.env.DATABASE_URL })

export const handler = async (event: APIGatewayEvent) => {
  const rows = await db.query('SELECT 1') // соединение уже готово
  return { statusCode: 200, body: JSON.stringify(rows) }
}
```

Для endpoint'ов, где пауза недопустима, включается provisioned concurrency: платформа держит заранее прогретый пул экземпляров и оплачивает его простой.

## Паттерны использования

- **Инициализация на уровне модуля** — подключение к БД, загрузка конфига и секретов выполняются один раз при старте.
- **Provisioned concurrency** — для критичных endpoint'ов держат тёплый пул экземпляров.
- **Лёгкий runtime и минимальный bundle** — чем меньше зависимостей, тем быстрее Init-фаза.
- **Регулярные warmup-запросы** — периодические ping'и не дают функции полностью остыть.

## Антипаттерны и ловушки

- **Инициализация внутри handler** — на каждый вызов заново коннект к БД и чтение секретов.
- **Тяжёлые зависимости в bundle** — cold start растёт пропорционально размеру артефакта.
- **Хранение состояния между вызовами в памяти** — экземпляр замораживается или пересоздаётся, состояние пропадает.
- **Неучёт cold start в SLO** — 800 мс на первое обращение неожиданно бьёт по перцентилям, если это не заложено в бюджет.

## Когда использовать / когда НЕ использовать

- **Использовать:** event-driven фоновые задачи, cron-джобы, webhooks, спорадические запросы, где пауза между вызовами допустима.
- **НЕ использовать:** latency-критичные endpoint'ы без provisioned concurrency; long-running процессы; сервисы, которым нужно постоянное состояние в оперативной памяти.

## Связанные темы

- **latency** — cold start вносит вклад в общую задержку ответа.
- **p99-latency** — хвост перцентиля во многом складывается из холодных стартов.
- **serverless-limitations** — холодный старт входит в список базовых ограничений Serverless.
