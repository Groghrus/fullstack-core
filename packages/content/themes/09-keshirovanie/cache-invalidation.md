...
После апдейта в БД приложение синхронно удаляет ключ и отдаёт свежий ответ — явная инвалидация в той же транзакции логики:

```ts
app.put('/users/:id', async (req, res) => {
  const { name } = req.body
  await db.query('UPDATE users SET name = $1 WHERE id = $2', [name, req.params.id])
  await redis.del(`users:${req.params.id}`)
  res.json({ ok: true })
})
```

Комбинация TTL (страховка) и explicit invalidation (закрытие окна устаревания) закрывает оба сценария: «забыл удалить ключ» и «пользователь ждёт свежие данные».

## Паттерны использования

- **TTL + явная инвалидация вместе** — TTL страхует от забытых команд, explicit invalidation закрывает окно несогласованности.
- **Версии в ключах** — `users:v2:<id>` позволяет сбросить группу перезаписью одного префикса без перебора ключей.
- **Инвалидация после коммита** — удалять ключ только после успешного коммита транзакции, иначе в кэш снова попадут старые данные.
- **Stale-while-revalidate** — на время пересчёта отдавать прошлую копию, чтобы не задерживать ответ.

## Антипаттерны и ловушки

- **Инвалидация до коммита** — читатель между удалением и записью снова положит в кэш устаревшее значение.
- **Очистка всего кэша на каждый апдейт** — «flush all» вызывает массовый кэш-шторм промахов и нагрузку на БД.
- **TTL как единственная стратегия** — допускает минуты устаревания для финансовых и транзакционных данных.
- **Отсутствие реакции на ошибку `DEL`** — Redis недоступен, БД уже обновлена: устаревшее значение живёт вечно.

## Когда использовать / когда НЕ использовать

- **Использовать:** любые данные, которые живут в кэше дольше одного запроса и меняются вне кэша (профили, каталоги, настройки), персонализированные страницы.
- **НЕ использовать:** счётчики и очереди, где Redis сам является источником истины; данные, умирающие строго по TTL без внешних изменений; когда допустима задержка секунды, а кэш дороже инвалидации.

## Связанные темы

- **caching-basics** — базовые стратегии кэширования, из которых вырастает инвалидация.
- **edge-caching** — как инвалидация касается распределённого кэша CDN.
