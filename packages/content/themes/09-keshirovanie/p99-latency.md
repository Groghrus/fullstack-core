...
Считаем перцентили на гистограмме, а не на сырой выборке — пример на Prometheus-гистограмме в Go:
...
Отчёт `histogram_quantile(0.99, ...)` даёт P99 напрямую; хранить все значения в памяти ради перцентилей не требуется — гистограмма использует O(1) памяти.

## Паттерны использования

- **Гистограммы вместо сырых значений** — память не растёт с числом запросов, точность настраивается бакетами.
- **P99 на уровне маршрута** — перцентиль в разрезе операции, а не по всему сервису сразу.
- **Отслеживать дрейф P99 между релизами** — ранний сигнал скрытой деградации.
- **Связывать P99 с нагрузкой** — всплеск при росте RPS указывает на saturation, а не на баг.

## Антипаттерны и ловушки

- **Среднее вместо перцентилей** — распределения задержек хвостаты, среднее их обманчиво.
- **P99 по всем данным сервиса** — в одну статистику смешиваются кэш-хиты и тяжёлые запросы.
- **Гистограмма без лейбла маршрута** — перцентиль считается по смеси разных операций.
- **Тюнинг P99 в отрыве от error rate** — оптимизация задержки ценой роста ошибок.

## Когда использовать / когда НЕ использовать

- **Использовать:** контроль SLO по времени ответа, сравнение версий, выявление хвостовых деградаций под нагрузкой.
- **НЕ использовать:** для редких фоновых job'ов (нет статистической базы); как единственную метрику без error rate и saturation.

## Связанные темы

- **latency** — базовое понятие задержки, из которого растёт перцентиль.
- **tail-latency** — что скрывается за P99 и как это лечить.
- **throughput** — вторая сторона нагрузки: сколько запросов выдерживает система.
