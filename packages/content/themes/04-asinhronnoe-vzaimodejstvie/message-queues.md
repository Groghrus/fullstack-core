...
- [x] Отдельная очередь для сообщений, которые не удалось обработать после max-попыток
- [ ] Быстрый кэш
- [ ] Уровень операционной системы

Пояснение: DLQ — место для «ядовитых» сообщений: их видно, можно отследить и обработать вне основного лучка.
