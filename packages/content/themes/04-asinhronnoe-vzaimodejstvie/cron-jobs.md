...
- **Фоновые процессы** — резолвер, почтовые рассылки по расписанию, синхронизация.
...
> Практика: **cron с расписанием**, **синхронная настройка K8s CronJob**, **реализация обёрток в Go/Java**.
...
- **Cron на каждой реплике без lock** — бэкап запускается N раз — дублирование.
...
- **Message Queues** — асинхронные сценарии через очередь вместо/вместе с cron.
