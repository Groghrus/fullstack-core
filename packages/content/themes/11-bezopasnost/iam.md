...
Проверка прав в middleware: роль пользователя сопоставляется с разрешением операции:
...
function requirePermission(perm: string) {
  return (req, res, next) => {
    ...
    if (!(RBAC[role]?.includes(perm) || RBAC[role]?.includes('*'))) {
...
Решение принимается в одном месте (middleware), а не «по соглашению» в каждом хендлере.

## Паттерны использования

- **Least privilege** — минимум прав, необходимый для работы роли.
- **Декларативные политики** — RBAC (роли→права), ABAC (атрибуты→условия) вместо размазанных проверок.
- **Принцип разделения** — аутентификация (кто) отдельно от авторизации (что можно).
- **Централизованное хранение политик** — одна точка изменения, а не N вложенных if'ов.

## Антипаттерны и ловушки

- **Дублирование проверок прав в каждом хендлере** — забытая проверка = дыра.
- **Связка «admin == владелец»** — роль привилегированная, а права на объект не проверяются.
- **Права, присваиваемые «когда удобно»** — со временем все становятся админами.
- **Проверка прав только на UI** — API остаётся открытым для тех, кто умеет curl.

## Когда использовать / когда НЕ использовать

- **Использовать:** многопользовательские приложения с ролями; аудит и комплаенс.
- **НЕ использовать:** для однопользовательских внутренних утилит — там достаточно аутентификации; избыточная авторизация усложняет поддержку.

## Связанные темы

- **oauth** — как внешняя аутентификация передаёт удостоверение системе.
- **jwt-rotation** — безопасная подпись и ротация токенов, которыми удостоверяется пользователь.
- **secret-management** — управление секретами, которые защищают механизмы доступа.
