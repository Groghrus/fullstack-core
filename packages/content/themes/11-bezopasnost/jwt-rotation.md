...
Верификация по JWKS-эндпоинту с учётом нескольких ключей подписи:
...
IdP держит пару ключей в JWKS: `kid` в заголовке токена указывает, каким проверить; отступление старого ключа позволяет валидировать ещё выпущенные токены.

## Паттерны использования

- **JWKS-эндпоинт и `kid` в токене** — ротация ключей проходит без деплоя у клиента.
- **Перекрытие поколений ключей** — show old key до истечения выпущенных токенов.
- **Короткие access, длинные refresh** — ротация refresh-токенов при каждом обновлении.
- **Асимметричные ключи** — подпись на IdP-сервере, верификация на любом клиенте публично.

## Антипаттерны и ловушки

- **Секрет в коде без ротации** — компрометация ключа делает всю подпись токенов пустой.
- **Жёстко зашитый `kid` / фиксированный ключ** — смена ключа требует деплой всех сервисов разом.
- **Не валидировать `alg`** — алгоритм-конфузия (RS256 против HS256) ломает верификацию.
- **Непроверка `aud`/`iss`** — токен действительно подписан, но не для вашего сервиса.

## Когда использовать / когда НЕ использовать

- **Использовать:** микросервисы с distributed верификацией; интеграция с IdP (OAuth 2.0/OIDC), где выпуск и проверка разделены.
- **НЕ использовать:** один процесс с собственным логином — сессия через cookie/серверное состояние проще и быстрее; JWT оправдан при разделении подписчика и верификатора.

## Связанные темы

- **oauth** — как выпускаются и используются токены в OAuth 2.0/OIDC.
- **iam** — последующий контроль доступа по удостоверенной личности.
- **secret-management** — управление ключами подписи как секретами.
