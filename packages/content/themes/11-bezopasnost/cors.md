...
Разрешаем только реальные домены фронтенда и типичные методы:
...
Ответ сервера получает заголовки `Access-Control-Allow-*`, и браузер пропускает ответ фронтенду только со списка.

## Паттерны использования

- **Белый список origin** — точные домены фронтенда, никаких wildcard для credentials.
- **Возможность лимитированных методов и заголовков** — минимум, необходимый приложению.
- **`credentials: true` только когда нужны куки/авторизация** — и тогда нельзя `*`.
- **Кэш preflight** — `Access-Control-Max-Age` уменьшает число лишних OPTIONS.

## Антипаттерны и ловушки

- **`Access-Control-Allow-Origin: *` при credentials** — браузер заблокирует ответ с куками.
- **Отражение `Origin` без проверки** — злоумышленник подставляет свой домен.
- **Доверять только CORS** — CORS защищает браузер, а не API: запросы из curl/SDK всё равно дойдут.
- **Не обрабатывать preflight OPTIONS** — SPA получит ошибку ещё до лишнего запроса.

## Когда использовать / когда НЕ использовать

- **Использовать:** публичный API, к которому ходят браузерные клиенты с других доменов.
- **НЕ использовать:** server-to-server вызовы (данные в заголовках, без браузера) — там CORS неприменим; разрешения управляются сетью и аутентификацией.

## Связанные темы

- **csrf** — перекрёстные запросы и защита от них тесно связаны с политикой origin.
