...
PagerDuty-ротация через Terraform — расписание дежурных описывается кодом:

```hcl
resource "pagerduty_schedule" "sre" {
  name = "SRE Rotation"

  layer {
    start = "2026-01-01T00:00:00Z"
    rotation_turn_length_seconds = 60 * 60 * 24 * 7
    restriction { type = "daily" }
    users = [
...
Эскалация и авто-ack настраиваются на тех же ресурсах — дежурный получает алерт, а если не отвечает 15 минут, пейдж передаётся secondary.

## Паттерны использования

- **Ротация со справедливым распределением** — недели/сутки по кругу, никто не дежурит вечно.
- **Эскалация вверх** — primary → secondary → тимлид с расписанными задачами.
- **Авто-ack и повторные будильники** — система не отпускает алерт, пока он не закрыт.
- **Тяжёлые алерты до on-call** — дежурный получает только реально критичное, остальное — в тикеты.

## Антипаттерны и ловушки

- **Дежурный без runbook'ов** — с экраном и алертом, но без инструкций.
- **Каждый алерт — звонок** — выгорание дежурных, алерты игнорируются (alert fatigue).
- **Ротация без отдыха** — дежурство ночью + рабочий день: сон и ревью нужны, иначе ошибки.
- **Single point — «дежурный — один и навсегда»** — отпуска и болезни ломают процесс.

## Когда использовать / когда НЕ использовать

- **Использовать:** production-сервисы с SLA; команды 2+ инженеров, чтобы ротация была честной.
- **НЕ использовать:** продукты без критичного трафика и реальных ночных инцидентов — ночные алерты в стоп-список, а дневные идут в тикеты.

## Связанные темы

- **production-incidents** — процесс, к которому подключается дежурный.
- **postmortems** — разбор после закрытия инцидента.
- **chaos-engineering** — эксперименты готовят дежурных к реальным сбоям.
