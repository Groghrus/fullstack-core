...
// Лёгкий стеммер: отсекаем типовые окончания — достаточно для tf-индекса.
const RU_SUFFIXES = [
...
/** Извлекает читаемый текст из markdown: без frontmatter, кода, служебных символов. */
...
export function buildIndex(inputs: SearchThemeInput[]): SearchIndex {
  const docs: SearchDoc[] = inputs.map((i) => ({
    id: i.id,
    title: i.title,
    blockId: i.blockId ?? '',
    path: i.path ?? '',
  ...
  const words: SearchIndex['words'] = {}
  docs.forEach((doc, d) => {
    const counts: Record<string, number> = {}
    for (const t of tokenize(doc.text)) counts[t] = (counts[t] ?? 0) + 1
    // Ключевые токены темы: заголовок и id-slug (например feature-flags →
    // 'flag'). Их индексируем всегда — тема находится даже без текста
    // (stub), и по английскому ключу, и по русскому названию.
...
export function search(
  ...
  const qTokens = tokenize(extractText(query))
